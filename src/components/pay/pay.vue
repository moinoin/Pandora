  ...
  <div class="pay">
    ...
    <div class="section">
      ...
      <div class="num order-message" v-for="item in orderPayList">
        ...
        <div class="res" @click="tostoreDetails(item.tbStores.storeId)">
          <div class="pic">
        ...
        <div class="good" v-for="(item,index) in item.orderPageDetails" @click="togoodDetails(item.tbGoods.goodsId)">
          <div class="good-left">
  ...
  export default {
    ...
    methods:{
      ...
      toorderDetails:function () {
        axios.get('http://39.108.161.47:8080/rener/order/receivePay',{
          params:{
            orderIds:this.ids
          }
        }).then(res => {
      ...
      // 初始化列表数据
      _initPayListData () {
        axios.get('http://39.108.161.47:8080/rener/order/pay').then(res => {
          console.log(res.data);
          if(res.data.payMsg.address.addressId==-1){
            alert('收货地址为空,请先填写收货地址!');
            this.$router.push({
              path:'./addAddress'
            })
          }
