...
//  功能按键和删除按键互换
  ...
  export default {
    ...
    methods:{
      _initOrderListData () {
        const car = 'http://39.108.161.47:8080/rener/goodsBasket/showBasket';
      ...
      numChange : function(index1, index, numChange) { //修改数量
        ...
        axios.get("http://39.108.161.47:8080/rener/goodsBasket/modifyCnt?",{
          params:{
            cnt:index1.count,
...
//        console.log(checkAllGood[0].checked);
      ...
      deletea:function(){  //删除购物车里的东西
        ...
        axios.get("http://39.108.161.47:8080/rener/goodsBasket/deleteGoods?",{
          params:{
      ...
      addppp:function(){   //去结算页面
        ...
        axios.get("http://39.108.161.47:8080/rener/order/save?",{
          params:{
            ids:this.goodCarIds
          }
        }).then(res =>{
