  ...
  export default {
    ...
    methods:{
      ...
      fullFavor:function () {
        ...
        let starURL = "http://39.108.161.47:8080/rener/colleState/changeState";
...
//          console.log(resp.data);
      ...
      fullCart:function () {
        ...
        let cartURL1 = "http://39.108.161.47:8080/rener/goodsBasket/addGoods";
        let cartURL2 = "http://39.108.161.47:8080/rener/goodsBasket/offMark";
        if(this.data3.state){
          if(btn1){
      ...
      _initComments (){
        let that = this;
        function getDetails(){
          return axios.get('http://39.108.161.47:8080/rener/goods/showOneGoods',{
            params : {
            goodsId : that.$route.query.goodsId
          }})
        }
        function getComments() {
          return axios.get('http://39.108.161.47:8080/rener/comment/selectAllByGoodsIdPlus',{params:{
            goodsId:that.$route.query.goodsId,
            page:0
          }})
        }
        axios.all([getDetails(),getComments()]).then(
          axios.spread(function(resp,com){
            ...
            let iconFavor = document.getElementsByClassName("icon-favor")[0];
            console.log(data1.colleState);

            if(data1.colleState == 0){
              iconFavor.style.top = -25 + 'px';
              if(btn3 && this.details.colleState === 0){
                iconFavor.style.top = -25 + "px";
                this.details.colleState = 1;
                btn3 = !btn3;
              }else if(!btn3 && this.details.colleState === 1){
                btn3 = !btn3;
                iconFavor.style.top = 0;
                this.details.colleState = 0;
              }
      ...
      _initData: function () {
        axios.get('http://39.108.161.47:8080/rener/nineGG/selectCounts').then(resp =>{
          let data = resp.data;
